...
돌려보기도 전에 @Autowired 에서 "Autowired members must be defined in valid Spring bean" 이라고 IDE가 노란 줄로 친절하게 알려줍니다.
...
의존성과 설정 파일 등에 따라 DataSource 를 찾지 못하거나 문법 오류 등에 대한 에러가 나타납니다.

따라서 Repository Test 를 수행할 때 나타나는 위의 에러들을 두 가지 경우에 대해서 알아보겠습니다.

---

## h2로 테스트를 수행하고자 하는 경우

실제 사용하는 DB 와는 상관없이 자주 사용하는 h2로 레포지토리 테스트를 간단하게 수행하고자 할 때가 있습니다.
...
### 테스트에 대한 설정 파일이 부재한 경우

테스트 시 테스트 패키지 내부에 설정 파일이 없으면 main 패키지 내부의 설정 파일을 읽습니다.

따라서 @DataJpaTest 에서 디폴트 값으로 등록한 h2 드라이버와 main 패키지 설정 파일의 DB 드라이버가 다를 시 sql 문법 에러를 반환합니다.
...
따라서 h2 를 [의존성에 추가](#h2-의존성을-추가하지-않은-경우)하고 [설정 파일에 추가](#테스트에-대한-설정-파일이-부재한-경우)하면 해당 h2 db 로 테스트를 진행합니다.

MySQL 로 설정 파일을 작성했다면 찾을 수 없기 때문에 에러가 발생합니다.

AutoConfigureTestDatabase 어노테이션 설명 부분의 See Also에 나와 있는 TestDatabaseAutoConfiguration 클래스를 보겠습니다.
...
앞서 @AutoConfigureTestDatabase 에서 Replace 값이 ANY 이기때문에 EmbeddedDataSourceBeanFactoryPostProcessor 가 빈으로 등록이 되고 Embedded Database 를 찾게 된 것입니다.
...
간단히 레포지토리 테스트를 MySQL 을 통해 수행할 수 있게 되었습니다.

MySQL 부분 설명은 [[JPA] DataJpaTest: 테스트시에 실제 DB 사용하기](https://kangwoojin.github.io/programing/auto-configure-test-database/) 를 보고 참고하여 작성하였습니다!